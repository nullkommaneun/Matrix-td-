/* ============================================================================
   Closure Hub — Boot/Diagnostics/Loader (boot.js)  v0.9.1
   ============================================================================ */
(function (global) {
  'use strict';
  ...
  // Debug-Flag: akzeptiert ?debug, ?debug=1, ?debug=true ODER localStorage('ch.debug') === '1'
  ...
  const Utils = CH.utils = {
    ...
    // Unicode-sichere Base64 (TextEncoder bevorzugt; Fallback auf escape/unescape)
    b64Encode(str) {
  ...
  class Logger {
    ...
    push(level, msg, data) {
      ...
      if (CH.config.debug) {
        const m = (level === 'ERROR' || level === 'FATAL') ? 'error' : (level === 'WARN' ? 'warn' : 'log');
        // eslint-disable-next-line no-console
        console[m](`[${level}]`, msg, data ?? '');
      }
      return line;
    }
    debug(m,d){return this.push('DEBUG',m,d);} info(m,d){return this.push('INFO',m,d);}
    warn(m,d){return this.push('WARN',m,d);}  error(m,d){return this.push('ERROR',m,d);}
    fatal(m,d){return this.push('FATAL',m,d);} asText(){return this.buf.map(x=>`${x.t} [${x.level}] ${x.msg}${x.data?' '+JSON.stringify(x.data):''}`).join('\n');}
    clear(){this.buf=[];}
  }
  ...
  const Loader = CH.loader = {
    ...
    check(name, ok, detail){
      ...
      if (!ok) this.health.ok = false; return ok;
    },
    assert(name, cond, detail){
      const ok = !!cond; this.check(name, ok, detail);
  ...
  const Diagnostics = CH.diagnostics = {
    ...
    recordError(err, label, extra){
      const e = { t: Utils.nowIso(), label: label||'unknown', message: String(err && err.message || err), stack: (err && err.stack)? String(err.stack): null, extra: extra ?? null };
      this.errors.push(e); CH.logger.error(`Caught error @${label||'unknown'}: ${e.message}`); CH.ui.flag(); return e;
    },
    generateMachineCode(){
      const env = { url: global.location.href, ua: navigator.userAgent, lang: navigator.language, platform: navigator.platform, screen: { w: global.innerWidth, h: global.innerHeight, dpr: global.devicePixelRatio||1 } };
      const state = (typeof this.stateSupplier === 'function') ? this.stateSupplier() : null;
      const payload = { meta: { ver: CH.VERSION, build: CH.BUILD, ts: Utils.nowIso() }, env, health: CH.loader.health, logs: CH.logger.buf, errors: this.errors, state };
    ...
    parseMachineCode(code){
      ...
      const parts = code.split('-'); const sig = parts[1]; const b64 = parts.slice(2).join('-');
      const json = Utils.b64Decode(b64); const calc = Utils.fnv1a(json).toString(16).padStart(8,'0');
      if (calc !== sig) throw new Error('Signaturprüfung fehlgeschlagen'); return JSON.parse(json);
    }
  };

  // --- Global Error-Capture --------------------------------------------------
  global.addEventListener('error', (ev)=>Diagnostics.recordError(ev.error||ev.message||'window.onerror','window.onerror'));
  global.addEventListener('unhandledrejection', (ev)=>Diagnostics.recordError(ev.reason||'unhandledrejection','unhandledrejection'));
    ...
    init(){ // idempotent
      ...
      if (btnCopy) btnCopy.addEventListener('click', CH.safeWrap(async ()=>{
        ...
        try{
          ...
          CH.logger.info('Maschinencode kopiert.');
        } catch(e){ CH.diagnostics.recordError(e,'debug-copy'); }
      }, this, 'debug-copy'));

      document.addEventListener('keydown', CH.safeWrap((e)=>{
        if ((e.ctrlKey||e.metaKey) && e.shiftKey && e.key.toLowerCase()==='d'){ e.preventDefault(); this.toggle(); }
      }, this, 'debug-hotkey'));
    },
