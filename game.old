/* ============================================================================
   ...
   - Aftercare-Pack (sanfte Karten)
   - Endbildschirm mit Reflexionsbericht, Sandbox-Weiter, Re-Seed, Export
   ============================================================================ */
...
function successProbability(state, def, dc) {
  ...
  return successes * 10; // 10 mögliche Ergebnisse → Prozent
}
...
function makeActions() {
  ...
  list.push(A({
    ...
    baseDC: 0, stat: Stat.Physique, energyCost: -10, stressDelta: -20, moneyDelta: 0, xpGain: 0, tags: ["rest", "stabilize", "aftercare"]
  }));
  list.push(A({
    id: "mindfulness_5", name: "5-Min Achtsamkeit", desc: "Auf Körper & Atem fokussieren.",
    baseDC: 0, stat: Stat.Physique, energyCost: -5, stressDelta: -15, moneyDelta: 0, xpGain: 0, tags: ["rest", "stabilize", "aftercare"]
  }));

  // Aftercare – sanfte Karten (neu)
  list.push(A({
    id: "aftercare_walk", name: "10-Min Spaziergang", desc: "Frische Luft, lockere Bewegung.",
    baseDC: 6, stat: Stat.Physique, energyCost: -8, stressDelta: -12, moneyDelta: +2, xpGain: 4, tags: ["aftercare", "stabilize"]
  }));
  list.push(A({
    id: "aftercare_music", name: "Musik-Session", desc: "2–3 Songs, die dich beruhigen.",
    baseDC: 6, stat: Stat.Appearance, energyCost: -6, stressDelta: -10, moneyDelta: +3, xpGain: 3, tags: ["aftercare", "stabilize"]
  }));
  list.push(A({
    id: "aftercare_checkin", name: "Check-in in 3 Sätzen", desc: "Wie geht’s? Was brauch ich? Was tu ich als Nächstes?",
    baseDC: 7, stat: Stat.Intellect, energyCost: 6, stressDelta: -6, moneyDelta: +4, xpGain: 6, tags: ["aftercare", "reflect", "stabilize"]
  }));
  list.push(A({
    id: "aftercare_gratitude", name: "3-Punkte Dankbarkeit", desc: "Drei kleine Dinge heute wertschätzen.",
    baseDC: 7, stat: Stat.Intellect, energyCost: 6, stressDelta: -6, moneyDelta: +5, xpGain: 6, tags: ["aftercare", "growth"]
  }));
...
function createState(meta, stats, perks, path, seedStr) {
  ...
  return {
    ...
    seed: seedStr,          // für Re-Seed-Neustart
    sandbox: false,         // Sandbox-Modus aktiv?
    noEnds: false,          // Enden/Milestones unterdrücken (Sandbox)
...
function pickOptions(state, pool) {
  ...
  for (const risk of risks) {
    ...
    const weights = candidates.map(def => {
      ...
      // Fortschritts-Priorisierung
      if (def.tags.includes("milestone-setup")) w *= 1.25;
      // Sandbox: Aftercare bevorzugen
  ...
  // Falls weniger als 3 gefunden, mit Aftercare/Erholung auffüllen
  const ids = new Set(out.map(o => o.def.id));
  function ensureSofteners() {
    const prefer = ["aftercare_walk", "aftercare_music", "aftercare_checkin", "aftercare_gratitude",
                    "breath_reset", "mindfulness_5"];
    for (const id of prefer) {
      const a = pool.find(x => x.id === id);
      if (a && !ids.has(a.id)) return { def: a, risk: Risk.Safe, targetDC: 0 };
    }
    return null;
  }
  while (out.length < 3) {
    const extra = ensureSofteners();
    if (!extra) break;
...
function applyAction(state, option) {
  ...
  // Endbedingungen (Sandbox unterdrückt Enden)
  if (!state.noEnds) {
    if (state.stress >= 100) {
    ...
    // Sandbox: Stress nicht über 99 steigen lassen
    state.stress = Math.min(state.stress, 99);
  }
  ...
  parts.push(`DC=${def.baseDC}, Stat=${def.stat}, Erfolgschance≈${prob}%`);
...
function checkMilestones(state) {
  if (state.noEnds) return; // Sandbox: keine neuen Meilensteine/Enden
...
/* ---------- Endscreen-Report / Export / Sandbox ---------- */
function buildReport(state) {
  const total = state.history.length || 1;
  const succ = state.history.filter(Boolean).length;
  const rate = Math.round((succ / total) * 100);

  const entries = [...state.successTags.entries()].sort((a, b) => b[1] - a[1]);
  ...
  return [
    `Erfolgsrate: ${rate}% (${succ}/${total})`,
    `Meilensteine: ${ms}`,
    `Wichtige Fortschritte: ${top}`,
    hints.length ? `Nächste Schritte:\n• ${hints.join("\n• ")}` : "Nächste Schritte: –"
  ].join("\n");
}
function sessionAsJSON(state) {
  const obj = {
    ...
    summary: {
      endTitle: state.endTitle, endSummary: state.endSummary,
      legacy: legacySummary(state)
    },
    stats: state.stats,
    resources: { energy: state.energy, stress: state.stress, hope: state.money, xp: state.xp },
    milestones: [...state.milestonesAchieved],
    successTags: Object.fromEntries(state.successTags),
...
function showEnd() {
  ...
  const rep = el("end-report");
  if (rep) setHTML(rep, buildReport(GAME));
  ...
  if (btnCont) btnCont.onclick = () => {
    // Sandbox: weiter spielen, Enden aus
    GAME.sandbox = true;
    GAME.noEnds = true;
    GAME.ended = false;
    GAME.energy = Math.max(GAME.energy, 30);
    GAME.stress = Math.min(GAME.stress, 95);
    elEnd.classList.add("hidden");
    elGame.classList.remove("hidden");
    refreshOptions(); renderStatus(); renderCooldowns();
  };
  if (btnReseed) btnReseed.onclick = () => {
    const same = GAME.seed || "" + Date.now();
    toStart();
    elSeedInput.value = same;
  };
  if (btnDl) btnDl.onclick = () => {
    const fname = `closure-hub_run_${(GAME.seed || 'seed')}.json`;
    downloadText(fname, sessionAsJSON(GAME));
  };
...
function toStart() {
  ...
  if (elFocus) elFocus.textContent = "Fokusmodus: AUS";
}
...
function optionCard(opt) {
  const def = opt.def;
  const prob = successProbability(GAME, def, def.baseDC); // 0..100
  const div = document.createElement("div");
  div.className = "option-card";

  const [stressAdj] = applyPerkEconomy(GAME.perks, def, def.stressDelta, def.moneyDelta, def.xpGain);
  const canAfford = (GAME.energy - def.energyCost) >= 0 &&
                    (GAME.stress + Math.max(0, stressAdj)) <= 100;

  div.innerHTML = `
    <h4 style="margin:0 0 6px;">
      ${def.name}
      <span class="badge">${String(opt.risk).toLowerCase()}</span>
    </h4>

    <div class="meta">
      Stat: ${def.stat} · Schwierigkeit (DC): ${def.baseDC} · Erfolgschance: ${prob}%
    </div>

    <div class="details" style="margin:6px 0 0;">
      <div class="muted">${def.desc}</div>
      <div class="meta">
        Kosten: Energie ${def.energyCost < 0 ? "+" + (-def.energyCost) : def.energyCost}
        · Stress ${signStr(def.stressDelta)}
        · Hoffnung ${signStr(def.moneyDelta)}
        · Einsicht +${def.xpGain}
      </div>
  ...
  // Fokusmodus: Details global ein-/ausblenden
...
on(elFocus, "click", () => {
  ...
  if (GAME) refreshOptions(); // Cards sofort neu rendern
});
